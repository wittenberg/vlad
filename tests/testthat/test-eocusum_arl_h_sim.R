...
test_that("Input parameter of function", {
  expect_error(eocusum_arl_h_sim(L0 = 0, k, df1, coeff1),
               "Given in-control ARL 'L0' must be a positive integer")
  expect_error(eocusum_arl_h_sim(L0, k = -1, df1, coeff1),
...
test_that("Different input values for df", {
  ...
  lapply(dftest1, function(x) {
    expect_error(do.call(x, eocusum_arl_h_sim(L0, k, df = x, coeff1)),
...
test_that("Different input values for coeff", {
   ...
   lapply(coefftest, function(x) {
     expect_error(do.call(x, eocusum_arl_h_sim(L0, k, df1, coeff = x)),
...
test_that("Iterative search procedure I (deteroration)", {
  ...
  works <- eocusum_arl_h_sim(L0=370, df=df1, k=kopt, m=m, coeff=coeff1, side="low", verbose=TRUE)
...
test_that("Iterative search procedure II (improvement)", {
  ...
  QA <- 1/2
  kopt <- optimal_k(QA=QA, df=df1, coeff=coeff1)
  works <- eocusum_arl_h_sim(L0=370, df=df1, k=kopt, m=m, coeff=coeff1, side="up", verbose=TRUE)
