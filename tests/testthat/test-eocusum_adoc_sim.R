...
test_that("Input parameter of function", {
  expect_error(eocusum_adoc_sim(r = 0, k, h, df1, coeff1, coeff2),
               "Number of simulation runs 'r' must be a positive integer")
  expect_error(eocusum_adoc_sim(r, k = -1, h, df1, coeff1, coeff2),
               "Reference value 'k' must be a positive numeric value")
  expect_error(eocusum_adoc_sim(r, k, h = 0, df1, coeff1, coeff2),
...
test_that("Different input values for df", {
  ...
  lapply(dftest1, function(x) {
    expect_error(do.call(x, eocusum_adoc_sim(r, k, h, df = x, coeff1, coeff2)),
                 "Provide a dataframe for argument 'df'")})

  dftest2 <- list(data.frame(0L, 1, 1), data.frame(0L), data.frame(NA))
  lapply(dftest2, function(x) {
    expect_error(do.call(x, eocusum_adoc_sim(r, k, h, df = x, coeff1, coef2)),
...
test_that("Different input values for coeff", {
  ...
  lapply(coefftest, function(x) {
    expect_error(do.call(x, eocusum_adoc_sim(r, k, h, df1, coeff = x, coeff2)),
...
test_that("Different input values for coeff2", {
  ...
  lapply(coefftest, function(x) {
    expect_error(do.call(x, eocusum_adoc_sim(r, k, h, df1, coeff1, coeff2 = x)),
...
test_that("Input parameter QS", {
  ...
  lapply(QStest, function(x) {
    expect_error(do.call(x, eocusum_adoc_sim(r, k, h, df1, coeff1, coeff2, QS = x)),
                 "QS must a positive numeric value")})
  expect_error(eocusum_adoc_sim(r, k, h, df1, coeff1, coeff2, QS = 1/2, side = "low"),
               "For detecting deterioration (side='low') QS must a positive numeric value >= 1",
               fixed = TRUE)
  expect_error(eocusum_adoc_sim(r, k, h, df1, coeff1, coeff2, QS = 2, side = "up"),
